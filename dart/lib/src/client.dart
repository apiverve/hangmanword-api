...
class HangmanwordValidationException implements Exception {
  final List<String> errors;

  HangmanwordValidationException(this.errors);

  @override
  String toString() => 'HangmanwordValidationException: ${errors.join("; ")}';
}
...
/// Hangman Word Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/hangmanword?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [category] - Word category
/// * [difficulty] - Difficulty level
/// * [count] - Number of words [min: 1, max: 10]
class HangmanwordClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'count': ValidationRule(type: 'integer', required: false, min: 1, max: 10),
  };

  HangmanwordClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/hangmanword',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HangmanwordValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HangmanwordValidationException(errors);
    }
  }

  /// Execute a request to the Hangman Word Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<HangmanwordResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HangmanwordException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HangmanwordResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HangmanwordException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HangmanwordException('Resource not found');
      } else {
        throw HangmanwordException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HangmanwordException) rethrow;
      throw HangmanwordException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Hangman Word Generator API client.
class HangmanwordException implements Exception {
  final String message;

  HangmanwordException(this.message);

  @override
  String toString() => 'HangmanwordException: $message';
}
